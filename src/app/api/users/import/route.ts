...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!firstConnection) {
      return NextResponse.json({ error: 'No apps connected to import users from' }, { status: 400 })
    }

    // 3. Get users from the accounting system via Integration.app
    const result = await client.connection(firstConnection.id).action('list-users').run()
