...
import { Connection, DataInput, Flow, useConnections, useIntegrationApp } from '@membranehq/react'
...
export function TriggerDialog({ open, onClose, onSubmit, node, mode = 'create' }: TriggerDialogProps) {
    ...
    const loadFlows = async (connection: Connection) => {
      ...
      try {
        ...
        if (mode === 'edit' && node?.flowKey) {
          ...
          if (trigger?.id) {
            try {
              const customerFlow = await integrationApp.connection(connection.id).flow(trigger.id).get()
  ...
  const handleTriggerChange = async (triggerKey: string) => {
    ...
    try {
      const customerFlow = await integrationApp.connection(state.connection.id).flow(trigger.id).get()
  ...
  const handleSubmit = async () => {
    ...
    try {
      await integrationApp.connection(state.connection.id).flow(state.flow.id).patch({ parameters: state.parameters })
