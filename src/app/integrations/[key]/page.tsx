...
export default function IntegrationConfigPage() {
  ...
  useEffect(() => {
    const fetchDataCollections = async () => {
      try {
        ...
        const collections = await integrationApp.integration(key as string).getDataCollections()
  ...
  const handleViewCollection = async (collection: any) => {
    try {
      ...
      const spec = await integrationApp.integration(key as string).getDataCollection(collection.key)
        ...
        <div className='flex items-center justify-between mb-6'>
          <div className='flex items-center space-x-4'>
            {integration.logoUri ? (
              ...
              <img src={integration.logoUri} alt={`${integration.name} logo`} className='w-12 h-12 rounded-lg' />
            ) : (
            ...
            <h1 className='text-3xl font-bold text-gray-900 dark:text-white'>{integration.name} Configuration</h1>
          </div>
        ...
        <div className='bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 mb-5'>
          <div className='space-y-4'>
            <div>
              ...
              <p className='text-gray-600 dark:text-gray-300'>Connection ID: {integration.connection.id}</p>
            </div>
          </div>
        </div>
                  ...
                  {dataCollections.map((collection) => (
                    ...
                    >
                      <div className='flex items-center justify-between'>
                        <h3 className='text-lg text-gray-900 dark:text-white'>{collection.name}</h3>
                        <Button variant='ghost' size='sm' onClick={() => handleViewCollection(collection)}>
                          View Details →
                        </Button>
                      </div>
                    </div>
