...
import { useConnections, useIntegrationApp, DataInput } from '@membranehq/react'
...
export function AddActionDialog({ open, onOpenChange }: AddActionDialogProps) {
  ...
  const handleSelectConnection = async (connection: any) => {
    try {
      ...
      const collections = await integrationApp.integration(connection.integration.key).getDataCollections()
    ...
    return (
      <div className='space-y-4 relative'>
        <div className='bg-gray-50 dark:bg-gray-900 rounded-lg p-4'>
          <h3 className='text-sm font-medium text-gray-900 dark:text-white mb-2'>Input:</h3>
          ...
          {selectedMethod ? (
            ...
            isLoadingCollections ? (
              ...
              dataCollections.map((collection) => (
                ...
                >
                  <div>
                    <h3 className='text-lg font-medium text-gray-900 dark:text-white'>{collection.name}</h3>
                  </div>
                  <Button variant='ghost' size='sm'>
                    Select →
                  </Button>
